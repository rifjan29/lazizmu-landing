<x-app-layout>
    ...
    @endpush
    @include('kategori-donasi.modal.create')
    @include('kategori-donasi.modal.show')
    @include('kategori-donasi.modal.edit')
    @push('js')
        <script>
            // show
            $('.show-data').on('click',function() {
                let id = $(this).data('id');
                $.ajax({
                    url: `{{ route('kategori-donasi.show', 1) }}`,
                    data: {
                        id: id
                    },
                    method: "GET",
            ...
            $('.edit-data').on('click',function() {
                let id = $(this).data('id');
                $.ajax({
                    url: `{{ route('kategori-donasi.edit', 1) }}`,
                    data: {
                        id: id
                    },
                    method: "GET",
                                        ...
                                        <td class="px-4 py-3 flex items-center justify-end">
                                            ...
                                            <div id="{{ $item->id }}-dropdown" class="hidden z-50 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="{{ $item->id }}-button">
                                                    ...
                                                    <li>
                                                        <a href="{{ route('kategori-donasi.destroy', $item->id) }}" data-confirm-delete="true" class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Delete</a>
                                                    </li>
                                                </ul>
                                                <div class="py-1">
